...
public class Game 
{
    ...
    private int randomNumber;//should coincide to the number of events in array list.
    private ArrayList<Event> events;

    private int eventProb;//how likely is any event to happen
    private int fightProb;//should be kept at 1
    ...
    private void createRoomsAndItems()
    {
        ...
        currentRoom = beach;  // start game on beach
    ...
    {
        Event TigerAttack = new Event("Tiger Attack","You have been attacked by a tiger",10, "fight");
        //events.add(TigerAttack);
        Event UnicornAttack = new Event("Unicorn Attack","You have been attacked by a unicorn",3,"fight");
        events.add(UnicornAttack);
        Event Illness = new Event("Ilness","You have influenza",5,"eat");
        events.add(Illness);
        Event Headache = new Event("Headache","You have a bad headache",1,"eat");
        events.add(Headache);
        Event NativeAttack = new Event("Native Attack","A group of natives spotted you and start pursuing you with bad intentions",8,"fight");
        events.add(NativeAttack);
    ...
    private void printHelp() 
    {
        ...
        System.out.println("around at the terrain.");
        System.out.println();
    ...
    private void goRoom(Command command) 
    {
        ...
        if (nextRoom == null) {
            ...
            boolean fire = trigger.fire(eventProb); 

            if(fire == true)
            {          
                getAttackInfo(trigger.getRandomNumber(randomNumber)); //picks a random number from 0-4 and passes it on to getAttackInfo              
     ...
     private void Eat(Command command){
        ...
        for(String thisItemName : keys) {
            ...
            if(thisItemName.equals("apple")){
                Item thisApple = itemsInPack.get(thisItemName);

                itemsInPack.remove(thisApple);
                System.out.println("You ate the apple"+ command.getSecondWord());
                System.out.println(printItemsInPack());
                return;
            }
            
             if(thisItemName.equals("chicken")){
                Item thisChicken = itemsInPack.get(thisItemName);

                itemsInPack.remove(thisChicken);
                System.out.println("You ate the Chicken"+ command.getSecondWord());
                System.out.println(printItemsInPack());
                return;
            }
            
             if(thisItemName.equals("pie")){
                Item thisPie = itemsInPack.get(thisItemName);

                itemsInPack.remove(thisPie);
                System.out.println("You ate the pie "+ command.getSecondWord());
                System.out.println(printItemsInPack());
                return;
            }
            
             if(thisItemName.equals("milkshake")){
                Item thisMilkshake = itemsInPack.get(thisItemName);

                itemsInPack.remove(thisMilkshake);
                System.out.println("You drank the milkshake "+ command.getSecondWord());
                System.out.println(printItemsInPack());
                return;
            }
    ...
    public void getAttackInfo(int index)//uses the number to get the object from events array list and asses the damage as well as options for flight or fight.
    {
        
        
            thisEvent = events.get(index); 
            System.out.println(thisEvent.getEventDescription());
            currentHealthNum = currentHealthNum - thisEvent.getDamage() ;
            System.out.println("Damage to your health is: - "+ thisEvent.getDamage() + " and depending on your health, results may be fatal.");
            System.out.println("If you want no damage you can choose to" +thisEvent.getOptions() +" and get back some of the points you lost, but you might not succeed.");
            return;
    ...
    private void useItem(Command command)
    {
        ...
        for(String thisItemName : keys) {
            ...
            if(thisItemName.equals(itemName)){
                ...
                boolean fire2 = trigger.fire(fightProb); 

                if(fire2 == true)
                {
                    int myNumber = trigger.getRandomNumber(10);
                    System.out.println("You just tried to use the "+ command.getSecondWord() +"to fight off the danger");
                    currentHealthNum = currentHealthNum + thisEvent.getDamage() ;
                    System.out.println("You have gained " + thisEvent.getDamage() + "points");
                    System.out.println(printItemsInPack());
                    return;
                }
                else
